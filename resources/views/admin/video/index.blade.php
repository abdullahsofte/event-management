                        ...
                        <div class="card">
                            ...
                            <form action="{{ route('video-gallery.store') }}" method="post">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-9">
                                        <textarea class="form-control my-form-control @error('youtube_link') is-invalid @enderror"  name="youtube_link" id="" cols="30" rows="4"></textarea>
                                          @error('youtube_link')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span> 
                                     ...
                                     <table id="datatablesSimple">
                                         ...
                                         <tbody>
                                            ...
                                            <tr>
                                                <td>{{ $key +1 }}</td>
                                                <td>{{ $item->title }}</td>
                                                <td><iframe width="100%" height="80" src="{{ asset($item->youtube_link) }}"></iframe></td>
                                                <td class="text-center">
                                                    <a href="{{ route('video-gallery.edit', $item->id) }}" class="btn btn-edit"><i class="fas fa-pencil-alt"></i></a>
                                                    <button type="submit" class="btn btn-delete" onclick="deleteUser({{ $item->id }})"><i class="far fa-trash-alt"></i></button>
                                                        <form id="delete-form-{{ $item->id }}" action="{{ route('video-gallery.delete', $item) }}"
